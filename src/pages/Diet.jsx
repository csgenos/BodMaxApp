  ...
  const handleAdd = async () => {
    ...
    try {
      await addDietEntry(profile.id, {
        date: todayStr(), meal: form.meal, calories: +form.calories,
        protein: form.protein ? +form.protein : 0, photo_url: form.photo,
  ...
  return (
    <div style={{ paddingBottom: 24 }}>
      <div style={{ padding: '52px 20px 20px', background: 'var(--bg2)', borderBottom: '1px solid var(--border)' }}>
        ...
        <MacroBar label="CALORIES" current={totalCal} target={profile?.target_calories} pct={calPct} color="var(--accent)" />
        <div style={{ marginTop: 10 }}>
          <MacroBar label="PROTEIN" current={`${totalProt}g`} target={`${profile?.target_protein}g`} pct={protPct} color="#4a9eb5" />
        </div>
      </div>
