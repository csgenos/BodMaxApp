...
import { getNotifPermission, requestNotifPermission, subscribePush, unsubscribePush, isPushSubscribed } from '../lib/notifications'
import { calcVolumes, getRank, getRankProgress, getNextTier, getTotalVolume, MUSCLE_GROUPS, TIERS } from '../lib/ranks'
  ...
  const handleNotifToggle = async () => {
    ...
    try {
      if (pushSubscribed) {
        ...
        const perm = notifPerm === 'granted' ? 'granted' : await requestNotifPermission()
        setNotifPerm(perm)
            ...
            {editing ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                <div style={{ display: 'flex', gap: 10, marginTop: 4 }}>
                  ...
                  <button onClick={saveProfileData} disabled={saving} style={{ flex: 2, background: 'var(--accent)', border: 'none', borderRadius: 'var(--radius-sm)', padding: 12, color: '#fff', fontWeight: 700, fontSize: 14 }}>{saving ? 'SAVING...' : 'SAVE'}</button>
                </div>
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {[
                  ...
                  ['PROTEIN TARGET', `${profile?.target_protein}g`],
                  ['WEIGHT UNIT', profile?.unit],
                  ...
                  <div className="card" style={{ padding: '12px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      ...
                      <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 3 }}>
                        {notifPerm === 'denied' ? 'Blocked in browser settings' : pushSubscribed ? 'Friend sessions & rest timer' : 'Off'}
                      </div>
                    </div>
                    {notifPerm !== 'denied' && (
                      <button
                        ...
                        style={{
                          ...
                          background: pushSubscribed ? 'var(--accent)' : 'var(--bg3)',
                ...
                {!showReset ? (
                  ...
                  <div style={{ background: 'var(--accent-low)', border: '1px solid var(--accent)', borderRadius: 'var(--radius)', padding: 16 }}>
                    <div style={{ fontSize: 13, color: 'var(--accent)', fontWeight: 600, marginBottom: 12 }}>⚠ This deletes all your data permanently</div>
                    <div style={{ display: 'flex', gap: 8 }}>
                      ...
                      <button onClick={signOut} style={{ flex: 1, background: 'var(--accent)', border: 'none', borderRadius: 'var(--radius-sm)', padding: 10, color: '#fff', fontWeight: 700 }}>CONFIRM</button>
                    </div>
                  </div>
                )}
              </div>
